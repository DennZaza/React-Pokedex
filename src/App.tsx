...
const PokemonList: React.FC<{ 
  ...
  // Sort Pokémon berdasarkan filter
  ...
  return (
    <>
      <div className="bg-red-500 min-h-screen">
        ...
        {/* Kotak Pokémon dengan efek scroll */}
        <div className="pokemon-container grid grid-cols-3 items-baseline content-start pb-3 border rounded-lg bg-white m-2 max-h-[calc(100vh-200px)] overflow-y-scroll">
...
const ProfileDropdown: React.FC<{ onFilter: (filter: 'name' | 'id') => void }> = ({ onFilter }) => {
  ...
  return (
    <div ref={dropdownRef} className="relative inline-block text-left">
      ...
      {isOpen && (
        <>
          <div className="fixed inset-0 bg-black opacity-50 z-40" onClick={toggleDropdown}></div>
          <div className="z-50 origin-top-right absolute right-0 mt-2 w-26 rounded-2xl shadow-lg bg-red-500 ring-1 ring-red-500 ring-opacity-5">
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPokemons = async () => {
      try {
        ...
        // Memperbarui state dengan data Pokémon yang telah diformat
        const enhancedPokemons = await Promise.all(
          allResults.map(async (result) => {
            const pokemonId = result.url.split('/').filter(Boolean).pop() || '0';
            ...
            return { ...result, id: Number(pokemonId), image: pokemonImage };
          })
        );
  ...
  // Mengubah filter pencarian
  const handleFilterChange = (newFilter: 'name' | 'id') => {
    setFilter(newFilter);
  };

  // Membersihkan input pencarian
